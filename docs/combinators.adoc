...
- `https://pkg.go.dev/github.com/purpleclay/chomp#pkg-variables:[IsDigit]`: Determines whether a rune is a decimal digit. A rune is classed as a digit if it is between the ASCII range of '0' or '9', or it belongs within the Unicode [Nd] category.
- `https://pkg.go.dev/github.com/purpleclay/chomp#pkg-variables:[IsLetter]`: Determines whether a rune is a letter. A rune is classed as a letter if it is between the ASCII range of 'a' and 'z' (including its uppercase equivalents), or it belongs within any of the Unicode letter categories: [Lu] [LI] [Lt] [Lm] [Lo].
